...
class Button:
    ...
    def show(self, screen: pygame.Surface):
        screen.blit(
            self.bg, (self.rect.x - self.imgOffets[0], self.rect.y - self.imgOffets[1])
        )

    def isClic(self, resoCible, resolution, offsets):
        if self.isActive and self.rect.collidepoint(
            get_scaled_mouse_pos(resoCible, resolution, offsets)
        ):
            self.function()
