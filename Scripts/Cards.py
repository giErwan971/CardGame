...
class Card:
    ...
    def show(self, screen : pygame.Surface, coords: tuple, bloomColor: tuple = (0, 0, 0, 0)):
        if bloomColor != (0, 0, 0, 0):
            bloomSurface = pygame.Surface((self.bg.get_width()+10, self.bg.get_height()+10), pygame.SRCALPHA)
            pygame.draw.rect(bloomSurface, bloomColor, bloomSurface.get_rect(), border_radius=8)
            screen.blit(bloomSurface, (coords[0]-5, coords[1]-5))
        screen.blit(self.bg, coords)

class Deck:
    def __init__(self, allCards: list[Card] = [], drawPile: list[Card] = [], onTable: list[list[Card]] = [], inHand: list[Card] = [], inOpponentHand: list[Card] = [], inDiscardPile: list[Card] = []):
    ...
    def saveDeck(self) -> "Deck":
        return Deck(self.allCards.copy(), self.drawPile.copy(), [pile.copy() for pile in self.onTable], self.inHand.copy(), self.inOpponentHand.copy(), self.inDiscardPile.copy())
